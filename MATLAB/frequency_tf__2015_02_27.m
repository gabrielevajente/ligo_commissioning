...
d = get_data({'H1:CAL-DELTAL_EXTERNAL_DQ', 'H1:IMC-F_OUT_DQ', 'H1:LSC-REFL_SERVO_ERR_OUT_DQ', 'H1:PSL-ISS_SECONDLOOP_SUM14_REL_OUT_DQ'}, 'raw', gps, dt);
fs0 = d(1).rate;
d0 = d(1).data;
...
np = fs0 * 16;
[sd0,f] = pwelch(d0, hanning(np), np/2, np, fs0);
sd0 = sqrt(sd0);
se0 = sqrt(pwelch(e0, hanning(np), np/2, np, fs0));
sc0 = sqrt(pwelch(c0, hanning(np), np/2, np, fs0));
si0 = sqrt(pwelch(i0, hanning(np), np/2, np, fs0));
