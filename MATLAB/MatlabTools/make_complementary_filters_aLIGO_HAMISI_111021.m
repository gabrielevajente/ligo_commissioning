function [lp_comp_filter, hp_comp_filter] = make_complementary_filters_aLIGO_HAMISI_111021(DOF)
%make_complementary_filters_aLIGO_HAMISI_111021(DOF)  makes complementary blend filters for aLIGO HAM ISI
% based on make_complementary_filters_H1HAM_090312 for x, y, z, rz
% and make_complementary_filters_H1HAM_10204 for rx and ry, but\
%     with extra DC zero (and low freq pole)
% taken from the filters designed by HAM_modelv3.m
%
% call as
% [lp_comp_filter, hp_comp_filter] = make_complementary_filters_aLIGO_HAMISI_111021(DOF)
...
%
%  updated from L1HAM_090119 by giving move rolloff of the displacement
% sensors so that the motion isn't increase between 1-10 Hz
% see plot by Corey Gray in LHO log of March 11, 2009
% 
% update by BTL on Oct 21, 2011 to merge filters and
% use minreal rather maketruecomplements 
...
elseif (strncmpi(DOF, 'X', 1) || strncmpi(DOF, 'Y', 1))
    ...
    X_disp_ellip = myellip_z2(.35,2,1,20,3)* 10^(1/20);
    X_more_poles = zpk([],-2*pi*[1.3,4*[1+1i, 1-1i]/sqrt(2)],(2*pi*1.3)*(2*pi*4)^2);


    X_LP_proto = X_disp_ellip * X_more_poles;

    X_HP_temp        = 1 - X_LP_proto;
    X_moreHP_filter  = myhpellip_z(.10,2,0.7,10,3)*10^(0.7/20);
    X_HPbutter       = myhpbutter(0.01,3);  % 0.01 Hz, 3 pole hp butterworth filter
