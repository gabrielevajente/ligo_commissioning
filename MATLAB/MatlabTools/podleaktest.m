function podleaktest(filename,numpods,chamberSNs)
...
headerlines = 25;
chamberSNs;
if numpods ~= length(chamberSNs)
    fprintf('WRONG NUMBER OF CHAMBERS! \n')
end
...
fprintf...
('Give me the time when valve was opened after Calibrated Leak accumulation. \n');
...
tcal = hour*3600 + minute*60 + second; % time to look for Neon calibrated leak


% two lines of code and half an hour later, output!
tcalstr = sprintf(' %G seconds', tcal);
disp(tcalstr)
...
[min_diff, indexcal] = min(abs(Time-tcal));

[Ncal, blank] = max(Neon(indexcal-20:indexcal+62)); 
[min_diff, Ncali] = min(abs(Neon-Ncal));

[Ncalbg, blank] = min(Neon(Ncali-15:Ncali));  % this gives the background 
[min_diff, Ncalbgi] = min(abs(Neon-Ncalbg));  % before
...
[min_diff, indexleak] = min(abs(Time-t1));

[PodM, blank] = max(Neon(indexleak-20:indexleak+62)); 
[min_diff, PodMi] = min(abs(Neon-PodM));

[Podbg, blank] = min(Neon(PodMi-15:PodMi));  % this gives the background 
[min_diff, Podbgi] = min(abs(Neon-Podbg));   % before
...
% fitt = Time(PodMi-10:end);
% exp(-fitt*x(3));
% fittedexp = y(1)+y(2)*exp(-fitt*x(3));
% Neon(PodMi-10:end);
% plot((10^14)*Neon(PodMi-10:end))
% hold on
% plot(fittedexp)
