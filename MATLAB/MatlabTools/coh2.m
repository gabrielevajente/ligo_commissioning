function [ coh, freq ] = coh2( time_series1, time_series2 , Ts, smooth_width, poly_fit_terms, win_pointer)
%coh2  calculate SMOOTHED, WINDOWED coherence between 2 time series
% 
%    [coherence, freq] = coh2(time_series1, time_series2, Ts, smooth_width, poly_fit_terms, win_pointer)
%
%     - required inputs-
...
%  
%     - outputs -
%    coherence   estimated coherence between the two signals, and
...
%         coh with bin width (freq resolution) of 50/(1000 sec) = 50 mHz
%
%    e.g. for data sets called st1z_time_series and st2z_time_series, 
%         which were saved at 1/2048 time steps, you might type:
%    [st1_2z_coh, freq] = coh2(st1z_time_series, st2z_time_series, 1/2048, 9, 1, @hann);
%
%    You can also call asd2 with only the first 2 or 3 or 4 inputs, e.g.     
%      [coherence, freq] = coh2(time_series1, time_series2, Ts)
%      [coherence, freq] = coh2(time_series1, time_series2, Ts, smooth_width)
%      [coherence, freq] = coh2(time_series1, time_series2, Ts, smooth_width, poly_fit_terms)
%
%    You can all call coh2 with [] as the inputs for:
%       smooth_width,  poly_fit_terms, and win_pointer, e.g.
%
%    [coherence, freq] = coh2(time_series1, time_series2, Ts, [], [], win_pointer)
%
%    inputs which are [] or not included will be set to the default values:
%    smooth_width = 9, poly_fit_terms = 1, and win_pointer = @hann.
%       e.g.
%    [coherence, freq] = coh2(time_series1, time_series2, Ts, [], 3); 
%       is equivalent to 
%    [coherence, freq] = coh2(time_series1, time_series2, Ts,  9, 3, @hann);
%   
%    coherence is dimensionless, and is valued between 0 and 1, 
%       it is defined as C12 = (<P12>.*<P21>) ./ (<P11>.*<P22>)
...
%       spectrum. The averaging is done before the multiplying.
%    
...
% 3) calculate PXX, PXY and PYY of each detrended, windowed, data
% 4) average PXX, PXY and PYY by adjecent bins into pxx, pyy, and pxy
...
PXY = 2 * (1/N)^2 * (1/BW) * (fX .* conj(fY));
clear fX

PYY = 2 * (1/N)^2 * (1/BW) * (fY .* conj(fY));
clear fY
%PYX = 2 * (1/N)^2 * (1/BW) * (fY .* conj(fX));
% we will not calculate PYX, because it is conj(PXY), and memory is
% finite! PXY .* PYX = abs(PX).^2
...
if smooth_width == 1 + 2*round((smooth_width-1)/2)  % is smooth_width odd?
    ...
    pyy(1) = mean(PYY(1:edge));
    pxy(1) = mean(PXY(1:edge));
    
    pxx(2:end) = mean(...
        reshape(PXX((edge+1):last_point_to_use), smooth_width, (new_fft_points-1)), 1);
    clear PXX
    pyy(2:end) = mean(...
        reshape(PYY((edge+1):last_point_to_use), smooth_width, (new_fft_points-1)), 1);
    clear PYY
    pxy(2:end) = mean(...
        reshape(PXY((edge+1):last_point_to_use), smooth_width, (new_fft_points-1)), 1);
    clear PXY
