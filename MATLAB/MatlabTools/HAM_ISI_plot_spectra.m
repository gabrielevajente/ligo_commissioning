%% HAM_ISI_plot_spectra
% Sept 18, 2014 - SB
% plot the calibrated Ground and GS13 spectra (m/rt(Hz) or rad/rt(Hz))
%
% HAM_ISI_plot_spectra(IFO,Chamber,start_time,duration)

function HAM_ISI_plot_spectra(IFO,Chamber,start_time,duration)


%% Data list

GRD_list={[IFO ':HPI-' Chamber '_STSINF_A_X_IN1_DQ'], ...
           [IFO ':HPI-' Chamber '_STSINF_A_Y_IN1_DQ'], ...
           [IFO ':HPI-' Chamber '_STSINF_A_Z_IN1_DQ'], ...
            };
...
%% Calibration
w_0=2*pi; Q=1/sqrt(2); GS13_num=[1 0 0 0]; GS13_den=[1 w_0/Q w_0^2];
...
for Counter=3:length(GS13_list)
    subplot(2,3,Counter)
   loglog(GS13_freq,abs(GS13_p(Counter,:)),'c','LineWidth',2)
    hold on
    loglog(GS13_freq,abs(GS13_noise),'m-','LineWidth',2)
    ...
    xlabel('Frequency (Hz)','FontSize',14)
    ylabel('ASD (m/rt(Hz) or rad/rt(Hz))','FontSize',14)
    legend_str2={GS13(Counter).name,'GS13 noise'};
    legend(legend_str2,'interpreter','none')
end

title(subplot(2,3,2),['ASD - ' Chamber],'FontSize',18)
