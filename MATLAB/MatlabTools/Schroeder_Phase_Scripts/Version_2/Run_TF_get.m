...
%get model rate using get_filenames which reads the simulink
%only works if IFO,SYS and CHAMBER have been added to the resp_stuct in
%Run_Get_Batch, if model_rate == 0 will default to the drive rate
try
    modelname = [resp_struct.IFO,resp_struct.SYS,resp_struct.CHAMBER];
    [filenames, DCUID, files_exist, model_rate] = get_filenames(modelname);
catch
    model_rate = 0;
    cprintf([0.3 0.4 0], '----Cannot find model (in run_tf_get) will use the drive rate for the model rate----\n');
     cprintf([0.3 0.4 0], 'In Run_Get_Batch  Please Initalize\n');
     cprintf([0.3 0.4 0], 'resp_struct.IFO = IFO;\n');
     cprintf([0.3 0.4 0], 'resp_struct.CHAMBER = Chamber;\n');
     cprintf([0.3 0.4 0], 'resp_struct.SYS =SYS;\n');
...
for n = 1:num_exc
    ...
    if resp_struct.control || isempty(strmatch('resp_chan_list',fields(param_exc(n)))) || isempty( param_exc(n).resp_chan_list)
     ...
     param_exc(n).model_rate = model_rate;
