  ...
  while  next_channel <= num_resp_chan
      ...
      while counter<100 && data_success == 0;
          ...
          try  % read the data from the frame-builder
              ...
              % of 2
              num_download_channels = floor((1E8/2)/(4*param.data_rate(1)*data_length)); %how man channels can we read at once
              ...
              for kk = 1:length(resp_data)
                  if sum(abs(resp_data{kk})) == 0
                      warning(['The data is all zeros!!   in channel ',param.resp_chan_list(current_down_load_channels(kk))]);
      ...
      for jj = 1:length(current_down_load_channels)
          ...
          result_n = compute_TF_temp(param, resp_data{jj}, exc_data,current_down_load_channels(jj));
