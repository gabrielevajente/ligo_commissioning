  ...
  for n = 1:num_resp_chan
    ...
    while counter<100 & data_success == 0;
      ...
      try
        ...
        [temp  param.data_rate(n)] = get_raw_data(param.resp_chan_list{n}, exc_start, ...
          1, param.data_rate(1), isVerbose);    

        resp_data  = get_raw_data(param.resp_chan_list{n}, exc_start, ...
          data_length, param.data_rate(n), isVerbose);  
%         [resp_data   param.data_rate(n)] = get_raw_data(param.resp_chan_list{n}, exc_start, ...
%           data_length, param.data_rate(1), isVerbose); % CR changed on 05/24/12 per Rich's M email on 04/12/12 
          data_success = 1;
   ...
   if length(resp_range) >= n
    ...
    result.ts_exc(:, n) = exc_data(:,1);
    if n == 16:21
        resp_data = decimate(resp_data,4,'fir')
    elseif n == 28:33
        resp_data =decimate(resp_data,4,'fir')
    end
%     result.ts_data(n).data(:,1) = resp_data(:);
    result.ts_data(:, n) = resp_data(:,1);
    end
  end
