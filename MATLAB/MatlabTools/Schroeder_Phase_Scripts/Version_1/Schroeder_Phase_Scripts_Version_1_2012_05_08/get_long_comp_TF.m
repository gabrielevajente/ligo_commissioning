...
%win_data = blackman(num_fft);
%num_overlap = round(0.75 * num_fft);
win_data = [];
num_overlap = [];

% detrend excitation and compute power spectrum
%a very inefficient way to get the frequency vecotr
...
f = 0:param.fRes:param.exc_rate/2; 
...
for n = 1:num_resp
  ...
  % transfer function and coherence
   tf(:, n)  = tfestimate(exc_data(n0:end), resp_data(n0:end, n), ...
    win_data, num_overlap, num_fft, param.data_rate(loop_num));
