...
% November 15, 2011 - VL
function Plot_Comparison_TF_ISI(TF_1_frd,TF_2_frd,Coh_1_frd,Coh_2_frd,Date_1,Date_2,Actuator,Sensor,Unit,Unit_ID,Unit_ID_Title,save_path,save_file_name,Autosave_Plot)

scrsz = get(0,'ScreenSize');
rads=180/pi;
Position_figure=[1 1 scrsz(3)/2 scrsz(4)/1.11]; %for double screen display
fontsize_gca=14;
fontsize_title=18;
fontsize_legend=12;
color=lines(12);
...
for Counter_1=1:3
    ...
    plot(Frequency_1,abs(TF_1_V),'Color',color(Counter_1,:),'LineWidth',2)
    plot(Frequency_2,abs(TF_2_V),'Color',color(Counter_1+4,:),'LineStyle','-.','LineWidth',2)
end
...
Unit_ID_str=Unit_ID_Title{:};
title_str_cat=strcat(Unit_ID_str(1:3),{' '},Unit_ID_str(5:7),{' '},Unit_ID_str(9:length(Unit_ID_str)),{' - '},{'Comparison transfer functions '}, {Date_1_cat},{' vs '}, {Date_2_cat});

legend(legend_str_cat,'FontSize',fontsize_legend,'Location','SouthWest');
title(title_str_cat,'FontSize',fontsize_title);

subplot(7,1,[4 5 6])
hold on
for Counter_1=1:3
    plot(Frequency_1,rads*angle(squeeze(TF_1(Sensor(Counter_1),Actuator(Counter_1),:))),'Color',color(Counter_1,:),'LineWidth',2)
    plot(Frequency_2,rads*angle(squeeze(TF_2(Sensor(Counter_1),Actuator(Counter_1),:))),'Color',color(Counter_1+4,:),'LineStyle','-.','LineWidth',2)
end
grid on
