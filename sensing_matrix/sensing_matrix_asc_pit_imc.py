...
filename = "asc_sensing_matrix_imc_pitch"

# where are you?
ifo = "H1" 

# list of excitation channels
excitations = ["SUS-MC1_M3_LOCK_P_EXC", "SUS-MC2_M3_LOCK_P_EXC", "SUS-MC3_M3_LOCK_P_EXC", "IMC-PZT_PIT_EXC"]
# sampling frequency of the excitation channels
fsamples = [16384, 16384, 16384, 2048]
# list of corresponding excitation frequencies
freqs = [8, 8, 8, 8, 8, 8, 8, 8]
# correction to the TF phase (for example due to downsampling)
phase_correction = -28.9
# and of amplitudes
...
monitors = ["SUS-MC1_M3_WIT_P_DQ", "SUS-MC2_M3_WIT_P_DQ", "SUS-MC3_M3_WIT_P_DQ", "IMC-PZT_PIT_OUT_DQ"]

# list of the error signal channels
readout = ["IMC-WFS_A_I_PIT_OUT_DQ", "IMC-WFS_B_I_PIT_OUT_DQ", "IMC-WFS_A_DC_PIT_OUT_DQ", "IMC-WFS_B_DC_PIT_OUT_DQ", 
           "IMC-MC2_TRANS_PIT_OUT_DQ", "IMC-IM4_TRANS_PIT_OUT_DQ", "IMC-IM4_TRANS_SUM_OUT_DQ"]

# Measurement time for each excitation
T = 120;

# Ramp time to switch on and off excitation
Tramp = 5
...
def excitation(channel, frequency, amplitude, T, Tramp, fs):
    # generate excitation file
    t = arange((T+2*Tramp)*fs)/double(fs)
    ...
    with open(os.devnull, 'w') as fp:
        subprocess.Popen(["awgstream", channel, str(fs), 'excitation.txt', str(amplitude)])
...
if len(sys.argv) == 1:
    ...
    logfile = []
    for exc,i,frequency,amplitude,fs in zip(excitations, range(len(excitations)), freqs, ampls, fsamples): 
        ### switch on excitation (with automatic timeout)
        sys.stdout.write("[%d] start excitation for channel %s\n" % (gpsnow(), exc))
        excitation(exc, frequency, amplitude, T, Tramp, fs)
...
for exc,mon,log,i,frequency in zip(excitations, monitors, logfile, range(len(excitations)), freqs): 
    ...
    for j in range(len(readout)):
        ...
        matrix[j,i] = cmy[fridx[0]] / pm[fridx[0]] * exp(1j*pi/180*phase_correction)
...
page.h1('Sensing matrix (abs, phase)')
page.table(border=1)
...
for j in range(len(readout)):
    ...
    for i in range(len(excitations)):
        ...
        if cohe[j,i] > cth:
            if i == idxmax:
                page.add("<b><font color=#ff0000>%e (%d)</font></b>" % (abs(matrix[j,i]), 180/pi*angle(matrix[j,i])))
            else:
                page.add("%e (%d)" % (abs(matrix[j,i]), 180/pi*angle(matrix[j,i])))
        else:
            page.add("<font color=#999999>%e (%d) </font>"  % (abs(matrix[j,i]), 180/pi*angle(matrix[j,i])))
        page.td.close()
    page.tr.close()
# close table
page.table.close()
